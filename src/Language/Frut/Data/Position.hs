...
maxPos p1@(Position a1 _ _) p2@(Position a2 _ _) =
  if a1 > a2 then p1 else p2
...
minPos p1@(Position a1 _ _) p2@(Position a2 _ _) =
  if a1 < a2 then p1 else p2
...
incPos p@Position {absoluteOffset = a, col = c} offset =
  p {absoluteOffset = a + offset, col = c + offset}

-- | Advance to the next line.
{-# INLINE retPos #-}
retPos :: Position -> Position
retPos NoPosition = NoPosition
retPos (Position a r _) =
  Position {absoluteOffset = a + 1, row = r + 1, col = 1}

-- | Advance only the absolute offset,
-- not the row and column information. Only use this if you
-- know what you are doing!
{-# INLINE incOffset #-}
incOffset :: Position -> Int -> Position
incOffset NoPosition _ = NoPosition
incOffset p@Position {absoluteOffset = a} offset =
  p {absoluteOffset = a + offset}
...
subsetOf (Span l1 h1) (Span l2 h2) =
  minPos l1 l2 == l1 && maxPos h1 h2 == h2
...
-- | Describes nodes that can be located -
-- their span can be extracted from them.
-- In general, we expect that for a value constructed
-- as @Con x y z@ where @Con@ is an arbitrary constructor
--
-- prop> (spanOf x <> spanOf y <> spanOf z) `subsetOf`
-- spanOf (Con x y z) == True
